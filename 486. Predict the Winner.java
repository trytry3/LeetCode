...
Finally, player 1 has more score (234) than player 2 (12), so you need to return True representing player1 ca
*/
...
class Solution {  
    public boolean PredictTheWinner(int[] nums) {
        ...
        // player 1 plays first, so can choose from nums[0] or nums[n-1]
    ...
    private int winner(int[] nums, int s, int e, Integer[][] memo) {
        ...
        if (memo[s][e] != null)
            return memo[s][e];
        // player chooses start
        int a = nums[s] - winner(nums, s + 1, e, memo);
        // player chooses end
        int b = nums[e] - winner(nums, s, e - 1, memo);
        // each player trying to maximize total score at that step
