...
class Solution {
    public int lengthLongestPath(String input) {
        ...
        for (String path : paths) {
            ...
            // +1 for adding /, -level because returned path has no \t
            // stack[level+1] stores the current path's length
            pathLens[level + 1] = pathLens[level] + path.length() - level + 1; 
            if(path.contains(".")) {
                maxLen = Math.max(maxLen, pathLens[level + 1] - 1); // -1 for removing /
            }    
        }
        return maxLen;
    }
