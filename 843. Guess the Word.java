...
class Solution {
    public void findSecretWord(String[] wordlist, Master master) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            // x is the number of matched chars to the secret word
            x = master.guess(guess);
            if (x == 6)
                return;
            // remove words that do not have x matched chars from wordlist
            List<String> wordlist2 = new ArrayList<>();
            for (String w : wordlist) {
                if (match(guess, w) == x) 
                    wordlist2.add(w);
            }
            // toArray() returns Object[], thus pass an argument to change its type
