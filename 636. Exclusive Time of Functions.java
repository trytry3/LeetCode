...
2. Your output should be sorted by function id, which means the 0th element of your output 
...
class Solution {
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        for(String log : logs) {
            String[] s = log.split(":");
            int fnId = Integer.valueOf(s[0]);
            int t = Integer.valueOf(s[2]);
        
            if(s[1].equals("start")) {
                stack.push(new int[]{fnId, t});
            } else {
                // end a function, pop that function's start out of stack
                int fnTime = t-stack.pop()[1]+1;
                res[fnId] += fnTime;
                // subtract the child function time from parent function execution time
                if(!stack.empty()) 
                    res[stack.peek()[0]] -= fnTime;
            }
        }
        return res;
    }
